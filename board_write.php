...
//$name1 = iconv("euc-kr","UTF-8",$name);
	...
	if (isset($_FILES[$imgName])) {
	    if (!is_dir($uploads_dir)) {
	        if (!mkdir($uploads_dir, 0777)) die("업로드 디렉토리 생성에 실패 했습니다.");
	        echo "<script> console.log('업로드 디렉토리 생성에 실패')</script>";
	    }
	    ...
	    if (!move_uploaded_file($_FILES[$imgName]['tmp_name'], $uploadFile)) { // 파일 이동
	...
	?>
	<script> console.log('값 넣기')</script>
	<?php
